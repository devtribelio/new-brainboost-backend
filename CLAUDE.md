...
- **KYC = Sumsub-driven disbursement gate (NEW rule, not legacy):** legacy had NO real KYC (`verification_kyc`/`last_kyc_status` were writer-less flags; `TBModule_Module_MemberDataKyc` doesn't exist). New flow: `POST /affiliate/me/kyc/token` mints a Sumsub SDK token (applicant `externalUserId` = member UUID, stored in `members.sumsub_applicant_id`); webhook `/api/webhook/sumsub` (HMAC raw-body digest) drives `kycStatus`: `applicantPending`→PENDING, `applicantReviewed` GREEN→APPROVED / RED→REJECTED. Disbursement still requires `kycStatus === 'APPROVED'`. Manual `POST /affiliate/me/kyc` kept as fallback. Spec: `docs/kyc-sumsub.md`.
