...
apps/
  ...
  notification-worker/     # ⚠️ EMPTY SHELL (node_modules only, no src) — FCM push runs
                           #   in-process from mobile-api; outbox relay = mobile-api
                           #   workers/comms-relay.ts → Amazon SQS (ADR-0002 said RabbitMQ;
                           #   actual transport is SQS, local dev = ElasticMQ)
...
| `tribelio-admin/` product-ops controllers + `application/tribelio/default/libraries/TBApi/Oracle/Method/**` + `controllers/api.php::oracle()` dispatcher | `src/modules/backoffice/` | NEW. JSON-only REST under `/api/backoffice/*` consumed by external backoffice SPA. Reuses `Admin` table + bearer JWT + RBAC (4 roles) + 2FA + audit log. Plan: `docs/specs/backoffice-port-plan.md` (+ per-cluster files in `docs/backoffice-port/`). |

For deep symbol-level mapping see `docs/specs/legacy-analysis.md`.
...
Module status (one-line summary; details in `docs/specs/rewrite-progress.md`):

- [x] auth — OAuth/JWT, register, forgot-password, devices, refresh grace window + supersession lineage (`docs/specs/refresh-token-grace.md`)
...
- [x] upload — multipart → S3 (sharp webp re-encode + resize + EXIF strip; `public/*` CDN, `private/*` presigned-ready). Replaces local disk + `/static/temporary`. See `docs/specs/upload-s3-port.md`
- [x] banner — list
...
- [x] notification — list, read, producer (commerce/post/comment/like/network), FCM v1 push (fire-and-forget), mute. Pending: FCM live credentials + manual push QA. RabbitMQ outbox deferred (see `docs/specs/notification-port.md §12`).
- [x] report — submit
- [x] app-version — public `GET /api/app/version-check` force/soft update verdict; replaces the app's Supabase `mobile_version_config` + store-listing scrape. See `docs/specs/app-version-check-contract.md`
- [x] admin — was done (EJS internal sysadmin), but `apps/admin-ejs` REMOVED 2026-07 (recover from git history if needed)
- [x] commerce / purchase — Xendit-only (CC + VA + eWallet), 2-step checkout→payment, voucher bypass, webhook + cron expire, event-driven side effects (enrollment + affiliate commission + voucher redeem). See `docs/specs/commerce-port.md`. Pending: manual Xendit sandbox QA
- [x] subscription — Phase 1 annual (BE-01…BE-22 semua selesai): schema+ledger, service aktivasi/renewal/grant, seats, entitlement+lazy enrollment, event bus+listeners, komisi flat, RC lifecycle, guard checkout, jobs reminder/expire, notif+email, modul HTTP `/subscription`, script grant+eligibility (2 sumber). 15 spec / 93 test. See `docs/specs/subscription-port.md`. Pending eksternal: SKU store+RC, rate renewal COO (template bb-comms DONE — BB-111)
- [ ] backoffice — `apps/backoffice-api` scaffold REMOVED 2026-07 (was never started). Plan docs kept: `docs/specs/backoffice-port-plan.md` + `docs/backoffice-port/`
...
- **Save discoveries to `docs/`** before clearing — anything non-obvious about legacy semantics, edge cases, or undocumented mobile-client expectations goes into `docs/specs/legacy-analysis.md` (or a new `docs/<feature>.md` if it's large).
- **Keep `docs/handbook/` in sync — ALWAYS, in the same change set.** Any change to the system MUST update the matching handbook page(s) before the work is considered done:
  - New/changed/dropped **DB table or column** → `docs/handbook/02-database.md` (domain table + ERD if relations changed) + the feature page that owns it.
  - New/changed/removed **endpoint or middleware** → rerun `pnpm docs:api` (regenerates `04-api-reference.md`) + update the endpoint table on the feature page.
  - Changed **business logic / rules / flow** → the feature page under `docs/handbook/features/` (rules list, Mermaid diagram if the flow changed).
  - New **event, job, worker, env var, or `app_settings` key** → `01-architecture.md` (event/job tables) and/or `operations/workers.md` / `operations/environment.md`.
  - New **module/feature** → new page under `features/` (follow the existing template: Overview → Endpoint → Tabel database → Flow → Business rules → Events & jobs → Referensi) + row in `docs/handbook/README.md` index.
  - Mermaid: always double-quote node labels & subgraph titles (unquoted `@`/`*`/`—` break the parser on GitHub).
...
- **End each session with:** "Update CLAUDE.md and docs/ with anything new discovered this session." Then act on it: append new business rules under §5, new dead-code findings to `docs/specs/legacy-analysis.md`, and bump module status in `docs/specs/rewrite-progress.md`.
...
- Run dev: `pnpm dev`
- Run tests: `pnpm test`
- Regenerate API reference handbook: `pnpm docs:api` (WAJIB setelah ubah routes — see §8)
- Technical handbook (must stay in sync, see §8): `docs/handbook/`
