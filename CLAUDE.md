...
apps/
  mobile-api/     :3000    # member-facing API. app.ts/main.ts/core/register-modules +
                           ...
                           #   workers/: comms-relay + push-relay (outbox -> SQS daemons)
  notification-worker/     # FCM push consumer (SQS notif-push -> fcmService), idempotent
                           #   via push_idempotency PK. Safe to scale out.
...
- **Topic post = rekap harian 21:00 WIB, BUKAN push instan (NEW rule, implemented 2026-07-29):** subscribe ke topic tidak memicu notifikasi apa pun ke pelanggan itu sendiri; nilainya adalah member ikut direkap saat ada post baru. Job `packages/domain/src/jobs/topic-digest-notifications.ts` (terdaftar di `jobs-runner.ts`, ikut lane hourly `bb-cron`) membaca tabel `posts` langsung atas window **24 jam tetap yang berakhir pada jam cutoff** (post setelah cutoff → rekap besok). **Jam kirim runtime-configurable:** `app_settings.notification.topicDigestHour` (0-23 WIB, key `SETTING_KEYS.notificationTopicDigestHour`, fallback+seed `DIGEST_HOUR_WIB`=21; nilai invalid → warn + default), lalu menulis **satu row per topic** (`ActionLabel.TopicDigest`, judul `"9 post baru di Topic A"`) dan mengirim **satu push gabungan** per member (>1 topic → `"12 post baru di 2 topic yang kamu ikuti"`) — karena itu producer punya `skipPush`. Idempoten via `dedupeKey = topicDigest:<hariWIB>:<topicId>:<memberId>`. **Job self-guarding pada jam WIB, bukan pada jam pemicu** — tidak ada TZ yang dipin di repo (PM2 pakai waktu lokal daemon), jadi job no-op sebelum boundary dan tetap benar walau TZ server salah. Hitungan mengecualikan post sendiri, `isDeleted`, non-published; mute dicek scope `topic` **dan** `network` induk. **Pengiriman push lewat outbox → SQS → worker:** job menulis row notifikasi + baris `notification_outbox` (`channel='fcm'`) dalam **satu `$transaction`** (ini menutup lubang durabilitas: dulu crash setelah row ditulis = push hilang permanen karena run berikutnya kena dedupe); `bb-push-relay` (`apps/mobile-api/src/workers/push-relay.ts`) mempublish ke queue `notif-push`; `apps/notification-worker` mengonsumsi, meng-claim `messageId` di tabel baru `push_idempotency` (PK, migration `20260729120000_add_push_idempotency`) **sebelum** memanggil FCM (SQS Standard = at-least-once), lalu kirim. Karena outbox-nya dipakai bersama comms, `comms-relay` **wajib** di-scope ke `COMMS_CHANNELS` — kalau tidak, baris push ikut terpublish ke queue comms. Queue + relay dipisah dari comms supaya fan-out bursty tidak mengantre di depan OTP. Poison message dihapus (tidak akan pernah sukses); kegagalan FCM di-throw → redeliver → DLQ. **Fallback:** `SQS_NOTIF_PUSH_URL` kosong → job kirim in-process seperti semula. Relay `instances:1`, worker aman di-scale out. Konsekuensi: `post.published` kini nol listener (event tetap dipertahankan). Yang instan tetap instan: comment/reply/tag/like, payment, komisi, join network. Butuh dukungan FE mobile untuk type baru `topicDigest`. Topic PRIVATE: join-request `PENDING` masih belum menotifikasi pemilik topic — gap yang diketahui, belum dikerjakan.
...
- [x] notification — list, read, producer (commerce/post/comment/like/network), FCM v1 push (fire-and-forget), mute (scope `post`/`network`/`topic`), **rekap topic harian 21:00 WIB** (`topicDigestNotifications`). Pending: FCM live credentials + manual push QA, dukungan FE untuk type `topicDigest`. RabbitMQ outbox deferred (see `docs/notification-port.md §12`).
- [x] report — submit
