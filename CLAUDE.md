...
- **Topic subscription = kanal push (NEW rule, implemented 2026-07-29):** subscribe ke topic **tidak** memicu notifikasi apa pun ke pelanggan itu sendiri; nilainya adalah member tersebut ikut ter-fan-out saat ada post baru di topic itu. `post.published` (emit di `post.service.ts` setelah `prisma.post.create`) sekarang membawa `topicId` dan dikonsumsi `packages/domain/src/notification/listeners/topic.listener.ts` → `RecipientResolver.resolveForTopic` → `ActionLabel.NewPost`, dedupe `newPost:<postId>:<memberId>`, author dikecualikan, mute dicek pada scope `topic` **dan** `network` induk. Sebelum ini `post.published` adalah dead event (nol listener) sehingga tidak ada push sama sekali dari jalur topic. Topic PRIVATE: join-request `PENDING` masih belum menotifikasi pemilik topic (asimetri vs alur network yang punya `requestJoin`/`approveJoin`) — gap yang diketahui, belum dikerjakan.
...
- [x] notification — list, read, producer (commerce/post/comment/like/network/**topic**), FCM v1 push (fire-and-forget), mute (scope `post`/`network`/`topic`). Pending: FCM live credentials + manual push QA. RabbitMQ outbox deferred (see `docs/notification-port.md §12`).
- [x] report — submit
