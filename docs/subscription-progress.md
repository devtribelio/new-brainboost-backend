...
- 2026-07-14 (BE-19, tambahan pasca-review): `GET /subscription/plans` + field `iosProductId`/`androidProductId`/`iosPrice` (paywall IAP; fallback ke `price` saat null) + **fix Swagger** — bentuk `type: () => [Dto]` tidak dikenali registry OpenAPI (schema response kosong diam-diam); pakai `isArray: true`. Detail di komentar BB-95.
