...
| BE-03 | SubscriptionService: aktivasi & renewal | `activateFromPayment` transaksional: create sub + pre-provision seat + owner claim seat 1; renewal = `max(now, expiresAt) + periodMonths` (atau expiry RC); idempoten via ledger; bump `expiredDate` lazy enrollment saat renewal. | L | BE-01 |
